"""
COBOL 定列格式·行级列处理（单一正本）——相1 预处理的最底层原语。

对应设计：docs/详细设计/步骤06-COBOL解析器列对齐与停用行修复设计.md（列处理逻辑正本）、
          docs/详细设计/步骤15-相1预处理独立成相设计.md §2.1（由 parser/cobol_columns.py 迁入相1）。

职责：对 COBOL 物理行做停用/注释/调试判定与列区清洗，是结构定位与构块/解析的共同基础。
迁入相1 后，正本归属 `preprocess.columns`；`parser/cobol_columns.py` 与 `decompose/lines.py`
转为薄重导出（保持原 import 路径不破），消除多套不一致的列处理。
